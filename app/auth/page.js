    ...
    <div style={{background:'#05070A',minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}>
      <div style={{background:'#11131A',border:'1px solid rgba(240,185,11,0.2)',borderRadius:'24px',padding:'40px 32px',width:'100%',maxWidth:'420px'}}>
        ...
        <form onSubmit={handleSubmit}>
          {!isLogin && <input type="text" placeholder="الاسم الكامل" value={name} onChange={e=>setName(e.target.value)} required style={{width:'100%',padding:'15px',borderRadius:'12px',border:'1px solid #2A2E3F',background:'#080A0F',color:'white',marginBottom:'16px',boxSizing:'border-box'}}/>}
          ...
          <button type="submit" disabled={loading} style={{width:'100%',padding:'15px',borderRadius:'12px',border:'none',background:'#F0B90B',color:'black',fontWeight:'900',cursor:'pointer'}}>{loading?'انتظر...':(isLogin?'دخول':'انشاء حساب')}</button>
        </form>
        {msg && <p style={{textAlign:'center',marginTop:'18px',color:'#F0B90B',fontSize:'13px'}}>{msg}</p>}
        <div style={{marginTop:'28px',textAlign:'center'}}><a href="/" style={{color:'#8B8FA3',textDecoration:'none',fontSize:'13px'}}>الرجوع للرئيسية</a></div>
      </div>
    </div>
  );
            }
